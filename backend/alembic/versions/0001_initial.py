...
def upgrade() -> None:
    # 1. Sab se pehle agar koi purani latchki hui type hai to use saaf karo
    op.execute("DROP TYPE IF EXISTS quotestatus CASCADE;")

    # 2. Direct table create karein, sa.Enum column khud ba khud background mein type create kar lega
    op.create_table(
        "quotes",
        sa.Column("id", sa.String(32), primary_key=True, index=True, nullable=False),
        sa.Column("client_name", sa.String(255), nullable=False),
        ...
        sa.Column(
            "status",
            sa.Enum("pending", "viewed", "contacted", name="quotestatus"), # create_type=False hata diya
            nullable=False,
            server_default="pending",
        ),
        sa.Column(
...
def downgrade() -> None:
    op.drop_table("quotes")
    # Clean rollback for the enum type
    op.execute("DROP TYPE IF EXISTS quotestatus CASCADE;")